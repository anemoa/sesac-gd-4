...
const CycleFuncChild = ({ number }) => {
    ...
    useEffect(() => {
        ...
        return () => {
            // Unmount 시점에 실행
            console.log('컴포넌트 언마운트! ❤️'); // 정리 함수: 언마운트 또는 다음 useEffect 실행 전 호출
        };
    ...
    // 의존성 배열이 [] 빈 배열이므로 #1, #2는 렌더링시 "한 번"만 실행
    ...
    useEffect(() => {
		...
		// - useEffect가 실행될 때, 이전 효과가 남아 있으면 중복된 효과가 발생할 수 있기 때문에
		// - 이전 효과를 먼저 제거한 후 새로운 효과를 적용하기 위함
    }, [input]);
    // input이 배열이 아니더라도 의존성 배열에 추가할 수 있다.
    // 문자열 숫자 상관없이 값이 변경될 때 마다 useEffect가 실행된다.

	// # 5. 여러개의 useEffect
	// - 보다시피 하나의 컴포넌트 안에 여러개의 useEffect 사용 가능
	// - React는 각각의 useEffect를 독립적으로 관리함.

    return (
        <div>
            <div>현재 숫자 값은 {number} 이다</div>
            <input
                type='text'
                value={input}
